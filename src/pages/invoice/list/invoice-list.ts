...
export class InvoiceListPage{
    ...
    setInvoiceListData(data) {
      ...
      if (data.invoices != null && data.invoices.length > 0) {
        for (let i = 0; i < data.invoices.length; i++) {
          this.mEmployeesList.push(data.invoices[i]);
        }
      }
  ...
  @Component({
    template: `
      <ion-list no-margin>
        <button ion-item no-lines (click)="editClientGroup()" *ngIf="invoiceUpdate && itemData.is_cancel.toLowerCase() == 'no' ">Edit</button>
        <button ion-item no-lines (click)="confirmDeleteEmployee()" *ngIf="invoiceDelete && itemData.is_cancel.toLowerCase() == 'no' ">Delete</button>
