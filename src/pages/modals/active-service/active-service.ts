...
export class ActiveServiceModel
{
  ...
  actionTaskSpentTime (post_params) {
    if (this.appConfig.hasConnection()) {
      ...
      this.clientDetailService.activeService(this.mItemData.id, post_params).then(data => {
        if (data != null) {
          ...
          if (this.apiResult.success) {
            this.appConfig.showNativeToast(this.appMsgConfig.ServicesActiveSuccess, "bottom", 3000);

            setTimeout(() => {
              this.eventCtrl.publish('service:active');
              this.dismiss();
            }, 500);
          } else {
